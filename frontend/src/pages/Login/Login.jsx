...
export default function Login() {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    let result = await login({ email: demoEmail, password: 'password' });
    if (result && result.success) {
      navigate('/dashboard');
      return;
    }
    // Auto-create demo account if not registered yet
    const regResult = await register({ fullName: `${provider} User`, email: demoEmail, password: 'password' });
      ...
      <div className="relative z-10 w-full max-w-[440px] rounded-3xl bg-[#0c1024]/85 p-8 border border-slate-800/80 shadow-2xl shadow-indigo-950/40 backdrop-blur-xl">
          ...
          <div className="relative mb-3 flex items-center justify-center">
            ...
            <div className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#0a0d1f] border border-indigo-500/40 p-2 shadow-inner">
              <svg viewBox="0 0 40 40" className="h-full w-full">
                ...
                <circle cx="20" cy="20" r="18" fill="none" stroke="url(#logoGrad)" strokeWidth="2.5" strokeDasharray="3 2" />
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/25 flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-50"
          >
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 bg-[#12162e]/80 hover:bg-[#1a2042] border border-slate-800/90 py-2.5 px-3 rounded-xl text-xs font-semibold text-slate-200 transition-all cursor-pointer"
          >
